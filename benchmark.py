...
from f1tenth_gym.maps.map_manager import TEST_MAPS_DICT as MAP_DICT
...
def main(cfg: DictConfig):
    ...
    model_path = os.path.join(cfg.model.ckpt)
    ...
    for ep in range(len(MAP_DICT)):
        ...
        while not done:
            ...
            with torch.no_grad():
                # スキャンをグラフに変換
                scan = obs['scans'][0]
                scan_tensor = torch.from_numpy(scan)     # shape: [1080]
                scan_tensor = scan_tensor.float()        # 必要に応じて dtype を float に
                scan_tensor = scan_tensor.unsqueeze(0)   #
                graph_batch = build_batch_graph(scan_tensor).to(device)
                # モデルの推論
                action = model(graph_batch).squeeze(0).cpu().numpy()
                steer = action[0] * cfg.dataset.max_steer # steer
                speed = linear_map(action[1], 0.0, 1.0, 0.0, 10.0) # speed
                action = [steer, speed]
